...
#[cfg(feature = "enc")]
pub fn simple_diff(older: &[u8], newer: &[u8], out: &mut [u8]) -> Result<usize, io::Error> {
...
pub fn simple_diff_with_params(
    ...
    use encoder::InstructionWriter;

    let mut writer = InstructionWriter::new(out);

    let mut translator = Translator::new(older, newer, |control| writer.write_control(control));
